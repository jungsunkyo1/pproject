...
기능적 요구사항
1.고객이 택시를 요청 한다
2.택시 요청 내역이 택시 기사에게 전달 된다.
3.택시 기사가 확인하여 요청을 접수 한다
4.택시 기사가 고객을 픽업하여 목적지까지 이동한다.
5.운행이 종료 되면 고객이 결제 한다.
6.택시 기사가 픽업하기 전에는 요청을 취소 할 수 있다.
7.고객이 요청 상태를 중간중간 조회한다.
8.요청 상태가 변경되면 카톡으로 알림을 보낸다.

비기능적 요구사항
1. 트랜잭션
    1. 취소된 요청은 택시 기사가 접수 할수 없어야 한다.  Sync 호출 
1. 장애격리
